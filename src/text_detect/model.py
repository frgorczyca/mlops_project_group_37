...
class LLMDetector(pl.LightningModule):
    ...
    def __init__(self, cfg):
        ...
        if self.optimizer_name == "adamw":
            ...
            logger.info(
                f"Using optimizer: {self.optimizer_name} with learning rate: {self.lr} and weight decay: {self.weight_decay}"
            )
        elif self.optimizer_name == "sgd":
            self.momentum = cfg.optimizer.momentum
            logger.info(
                f"Using optimizer: {self.optimizer_name} with learning rate: {self.lr} and momentum: {self.momentum}"
            )
        else:
        ...
        self.class_names = (
            ["Human", "AI"] if cfg.data.num_classes == 2 else [f"Class_{i}" for i in range(cfg.data.num_classes)]
        )
    ...
    def _shared_step(self, batch, batch_idx, step_type):
        ...
        try:
            ...
            if step_type in ["val", "test"]:
                getattr(self, f"{step_type}_outputs").append({"preds": preds, "labels": labels, "probs": probs})
        ...
        self.logger.experiment.log(
            {
                ...
                f"{stage}_pr_curve": wandb.plot.pr_curve(
                    all_labels.cpu().numpy(), all_probs.cpu().numpy(), labels=self.class_names
                ),
